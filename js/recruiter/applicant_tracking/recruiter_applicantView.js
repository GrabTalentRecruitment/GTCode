$(function(){
    ...
    // This is the modal that shows all the action buttons. 
    $('#candStepModal').on('show.bs.modal', function (event) {
        ...
        var tmp = button.attr('title').split('/');
        var candprevStg = tmp[0];
        jobRefId = tmp[1];
    ...
    $("button#Reject_btn").click(function (event) {
        ...
        var tmpcandprevStg = $(this).attr('title');
        var candprevStgJob = tmpcandprevStg.split('/');
        ...
        if( confirmCandRejFunc() != 0) {
            ...
            $.ajax({
                ...
                data        : { 'candidateprevStepname' : candprevStgJob[0], 'candidateJobId' : candprevStgJob[1], 'candidateRefCode' : candidaterefCode, 'candStepname' : 'Rejected'}, // our data object
                crossDomain : true
            })
            .done(function(data) {
                var response = data.split(':');
